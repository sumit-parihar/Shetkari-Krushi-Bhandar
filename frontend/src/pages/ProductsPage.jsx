...
export default function ProductsPage() {
  ...
  // Local input states — kept in sync with URL on mount
  const [inputKeyword, setInputKeyword] = useState(keyword)
  const [localMin, setLocalMin] = useState(min_price)
  const [localMax, setLocalMax] = useState(max_price)

  // Fix 3: Debounce ref so search bar works correctly without stale closures
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 animate-fade-in">
      <div className="mb-6">
        ...
        <p className="text-earth-500 mt-1">Quality agricultural products for every need</p>
      </div>
      ...
      {filtersOpen && (
        <div className="card p-5 mb-6 animate-slide-up">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            {/* Category */}
            <div>
              <label className="block text-xs font-semibold text-earth-600 mb-2 uppercase tracking-wide">Category</label>
            ...
            {/* Price Range */}
            <div>
              <label className="block text-xs font-semibold text-earth-600 mb-2 uppercase tracking-wide">Price Range (₹)</label>
            ...
            {/* Sort */}
            <div>
              <label className="block text-xs font-semibold text-earth-600 mb-2 uppercase tracking-wide">Sort By Price</label>
              <div className="flex gap-2">
                {[['', 'Default'], ['asc', 'Low → High'], ['desc', 'High → Low']].map(([val, label]) => (
                  <button
                    key={val}
      ...
      {!loading && (
        <p className="text-sm text-earth-500 mb-4">
          {total > 0
            ? `${total} product${total !== 1 ? 's' : ''} found${keyword ? ` for "${keyword}"` : ''}`
            : hasFilters ? 'No products match your filters' : ''
          }
        </p>
      )}
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          desc={hasFilters ? 'Try adjusting your search or filters' : 'No products available right now'}
          action={hasFilters ? <button onClick={clearAll} className="btn-secondary text-sm">Clear filters</button> : null}
        />
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
