...
export default function ProductDetailPage() {
  ...
  if (error) return (
    <div className="max-w-xl mx-auto px-4 py-16">
      ...
      <Link to="/products" className="btn-secondary mt-4 inline-flex">← Back to Products</Link>
    </div>
  )
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 animate-fade-in">
      ...
      <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-center gap-2">
            {isOutOfStock ? (
              ...
              <span className="flex items-center gap-1.5 text-amber-600 text-sm font-medium">
                <AlertTriangle className="w-4 h-4" /> Only {product.stock_quantity} left
              </span>
            ) : (
              <span className="flex items-center gap-1.5 text-leaf-600 text-sm font-medium">
                <CheckCircle className="w-4 h-4" /> In Stock ({product.stock_quantity} units)
              </span>
            )}
          </div>
          ...
          {!isOutOfStock && isCustomer && (
            <div className="space-y-3 pt-2">
              <div className="flex items-center gap-3">
                <label className="text-sm font-medium text-bark">Quantity:</label>
              ...
              <button onClick={handleAdd} disabled={adding} className="btn-primary w-full py-3.5 text-base">
                {adding ? <Spinner size="sm" /> : <><ShoppingCart className="w-5 h-5" /> Add to Cart</>}
              </button>
            </div>
          )}
          ...
          {/* Fix 2: Removed Product ID box. Only show Category. */}
          {product.category_name && (
            <div className="pt-4 border-t border-earth-100">
              <div className="bg-earth-50 rounded-lg p-3 inline-flex items-center gap-2">
                ...
                <p className="text-xs text-earth-500">Category: <span className="font-semibold text-bark">{product.category_name}</span></p>
              </div>
            </div>
          )}
        </div>
