    ...
    <div className="p-6 animate-fade-in space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-earth-500 text-sm mt-0.5">Last 12 months performance overview</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Revenue',   value: formatCurrency(totalRevenue), icon: TrendingUp,  color: 'bg-leaf-50 text-leaf-700 border-leaf-200' },
          { label: 'Total Orders',    value: totalOrders,                  icon: ShoppingBag, color: 'bg-earth-50 text-earth-700 border-earth-200' },
          { label: 'Delivered',       value: totalDelivered,               icon: Truck,       color: 'bg-blue-50 text-blue-700 border-blue-200' },
          { label: 'Cancelled',       value: totalCancelled,               icon: Package,     color: 'bg-red-50 text-red-700 border-red-200' },
        ].map(c => (
      ...
      {chartData.length > 0 && (
        <div className="card p-5">
          <h2 className="font-display font-semibold text-bark mb-4 flex items-center gap-2">
            <TrendingUp className="w-4 h-4 text-leaf-600" /> Monthly Revenue (₹)
          </h2>
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={chartData} margin={{ top: 4, right: 8, left: -10, bottom: 0 }}>
              ...
              />
              <Legend formatter={n => n === 'revenue' ? 'Revenue' : 'Orders'} />
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="card p-5">
          <h2 className="font-display font-semibold text-bark mb-4 flex items-center gap-2">
            <Package className="w-4 h-4 text-leaf-600" /> Top Products by Units Sold
          </h2>
          {data?.top_products?.length === 0 ? (
            <p className="text-earth-400 text-sm text-center py-6">No data yet</p>
          ) : (
                ...
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between mb-0.5">
                      ...
                      <span className="text-xs text-earth-500 shrink-0 ml-2">{p.total_sold} units</span>
                    </div>
        ...
        <div className="card p-5">
          <h2 className="font-display font-semibold text-bark mb-4 flex items-center gap-2">
            <Tag className="w-4 h-4 text-leaf-600" /> Revenue by Category
          </h2>
      ...
      {data?.delivery_perf?.length > 0 && (
        <div className="card p-5">
          <h2 className="font-display font-semibold text-bark mb-4 flex items-center gap-2">
            <Truck className="w-4 h-4 text-leaf-600" /> Delivery Boy Performance
          </h2>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[400px]">
              <thead>
                <tr>
                  {['Delivery Boy', 'Total Assigned', 'Delivered', 'Completion Rate'].map(h => (
