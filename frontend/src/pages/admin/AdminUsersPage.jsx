  ...
  return (
    <div className="p-6 animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="section-title">{t('admin.users.title')}</h1>
          <p className="text-earth-500 text-sm mt-0.5">{t('admin.users.total', { count: total })}</p>
        </div>
      </div>
        ...
        <table className="w-full min-w-[640px]">
          ...
          <tbody>
            {loading
              ...
              : users.length === 0
              ? <tr><td colSpan={6}><EmptyState title={t('admin.users.noUsers')} icon={Users} /></td></tr>
              : users.map(u => (
                <tr key={u.user_id} className="hover:bg-earth-50/50 transition-colors">
                  ...
                  <td className="table-cell">
                    <div className="flex items-center gap-2">
                      ...
                      {u.user_id === currentUser?.user_id && (
                        <span className="text-[10px] bg-earth-100 text-earth-600 px-1.5 py-0.5 rounded font-medium">
                          {t('admin.users.you')}
                        </span>
                      )}
                    </div>
                  ...
                  <td className="table-cell">
                    <span className={`badge text-xs ${
                      u.role === 'admin'        ? 'bg-soil-100 text-soil-700' :
                      u.role === 'delivery_boy' ? 'bg-blue-100 text-blue-700' :
                                                  'bg-leaf-100 text-leaf-700'
                    }`}>
                      {u.role === 'delivery_boy' ? t('admin.users.deliveryBoy') :
                       u.role === 'admin'         ? t('admin.users.admin') :
                                                    t('admin.users.customer')}
                    </span>
                  </td>
                  ...
                  <td className="table-cell">
                    <div className="flex items-center gap-1">
                      <button onClick={() => openEdit(u)}
                        className="p-1.5 rounded-lg hover:bg-earth-100 text-earth-400 hover:text-leaf-700 transition-colors">
                        <Pencil className="w-3.5 h-3.5" />
                      </button>
                      {u.user_id !== currentUser?.user_id && (
                        <button onClick={() => setDeleteId(u.user_id)}
                          className="p-1.5 rounded-lg hover:bg-red-50 text-earth-400 hover:text-red-600 transition-colors">
      ...
      {/* Edit Modal */}
      <Modal open={!!editUser} onClose={() => setEditUser(null)} title={t('admin.users.editUser')} size="md">
        <form onSubmit={handleUpdate} className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1.5">{t('admin.users.nameLabel')}</label>
              <input className="input-field" value={form.name}
                onChange={e => setForm(p => ({...p, name: e.target.value}))} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1.5">{t('admin.users.emailLabel')}</label>
              <input type="email" className="input-field" value={form.email}
                onChange={e => setForm(p => ({...p, email: e.target.value}))} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1.5">{t('admin.users.roleLabel')}</label>
              <select className="input-field" value={form.role}
                onChange={e => setForm(p => ({...p, role: e.target.value}))}>
                <option value="customer">{t('admin.users.customer')}</option>
                <option value="admin">{t('admin.users.admin')}</option>
                <option value="delivery_boy">{t('admin.users.deliveryBoy')}</option>
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium mb-1.5">{t('admin.users.phone')}</label>
              <input className="input-field" value={form.phone}
                onChange={e => setForm(p => ({...p, phone: e.target.value}))}
                placeholder={t('admin.users.optional')} />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium mb-1.5">{t('admin.users.address')}</label>
          ...
          <div className="flex gap-3 justify-end pt-1">
            ...
            <button type="submit" disabled={submitting} className="btn-primary">
              {submitting ? <Spinner size="sm" /> : t('admin.users.saveChanges')}
            </button>
          </div>
        </form>
      </Modal>
