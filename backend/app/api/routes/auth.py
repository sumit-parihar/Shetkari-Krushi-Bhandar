...
def _validate_register_fields(data: dict) -> str | None:
    ...
    if len(email) > MAX_EMAIL:
        return f"email must be {MAX_EMAIL} characters or fewer"
    ...
    if len(password) > MAX_PASSWORD:
        return f"password must be {MAX_PASSWORD} characters or fewer"
    if phone and len(phone) > MAX_PHONE:
        return f"phone must be {MAX_PHONE} characters or fewer"
    if address and len(address) > MAX_ADDRESS:
        return f"address must be {MAX_ADDRESS} characters or fewer"
    return None
...
async def register(request: Request, conn=Depends(get_db)):
    ...
    err = _validate_register_fields(data)
    if err:
    ...
    try:
        ...
        conn.execute(
            ...
            (data["name"].strip(), data["email"].strip(), hashed_password, "customer", phone, address),
        )
        conn.commit()
...
async def update_own_profile(request: Request, conn=Depends(get_db), user=Depends(require_user)):
    ...
    try:
        ...
        if len(name) > MAX_NAME:
            return error_response(f"Name must be {MAX_NAME} characters or fewer", 400)
        if not email or "@" not in email:
            return error_response("Valid email is required", 400)
        if len(email) > MAX_EMAIL:
            return error_response(f"Email must be {MAX_EMAIL} characters or fewer", 400)
        ...
        if phone and len(phone) > MAX_PHONE:
            return error_response(f"Phone must be {MAX_PHONE} characters or fewer", 400)
        if address and len(address) > MAX_ADDRESS:
            return error_response(f"Address must be {MAX_ADDRESS} characters or fewer", 400)
        ...
        logger.exception("Profile update failed for user_id: %s", user_id)
        conn.rollback()
        return error_response("Internal server error", 500)
...
async def change_password(request: Request, conn=Depends(get_db), user=Depends(require_user)):
    ...
    if len(new_password) > MAX_PASSWORD:
        return error_response(f"Password must be {MAX_PASSWORD} characters or fewer", 400)
...
async def list_users(request: Request, conn=Depends(get_db), admin=Depends(require_admin)):
    ...
    try:
        ...
        return success_response(
            data={"users": users_out, "page": page, "page_size": page_size, "total": total}
        )
...
async def admin_update_user(
    user_id: int, request: Request, conn=Depends(get_db), admin=Depends(require_admin)
):
    try:
        data = await request.json()
    except Exception:
    ...
    try:
        ...
        if len(name)  > MAX_NAME:  return error_response(f"Name max {MAX_NAME} chars",    400)
        if len(email) > MAX_EMAIL: return error_response(f"Email max {MAX_EMAIL} chars",   400)
        ...
        conn.execute(
            "UPDATE Users SET name=?, email=?, role=?, phone=?, address=? WHERE user_id=?",
